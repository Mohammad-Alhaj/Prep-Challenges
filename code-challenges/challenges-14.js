...
// let re = result.slice(result.length-1);
...
const replaceWords = (str) => {
    ...
    let i = result.indexOf('I');

    let am = result.indexOf('am');

    let was = result.indexOf('was');

    console.log(result.indexOf('I'));

    result.splice(i, 1, 'We');

    result.splice(am, 1, 'are');

    result.splice(was, 1, 'were');

    let resu2 = str.split(" ");

    result.splice(result.length-1, 1, resu2[resu2.length-1]);

    let news = result.join(" ");
